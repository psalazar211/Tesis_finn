...
// Forward declarations (needed because templates use these before definition)
inline void dq42c1(ap_uint<1> x1, ap_uint<1> x2, ap_uint<1> x3, ap_uint<1> x4,
    ap_uint<1>& sum, ap_uint<1>& carry, ap_uint<1>& Cout);

inline void dq42c2(ap_uint<1> x1, ap_uint<1> x2, ap_uint<1> x3, ap_uint<1> x4,
    ap_uint<1>& sum, ap_uint<1>& carry, ap_uint<1>& Cout);
...
#pragma HLS loop_tripcount min=0 max=64
            ...
            if (cnt[col] >= 4) {
                ...
                heap[col][cnt[col]++] = s;
                heap[col + 1][cnt[col + 1]++] = c;
                if (co) heap[col + 1][cnt[col + 1]++] = co;  // C2 uses Cout=x3
            }
            else {
                // FA exacto 3:2 fallback
                ap_uint<1> x3 = heap[col][--cnt[col]];
                ap_uint<1> x2 = heap[col][--cnt[col]];
                ap_uint<1> x1 = heap[col][--cnt[col]];
                ap_uint<1> s = x1 ^ x2 ^ x3;
...
// ------------------------------------------------------------
// DQ 4:2 compressors (approx-mode behavior)
// ------------------------------------------------------------
inline void dq42c1(ap_uint<1> x1, ap_uint<1> x2, ap_uint<1> x3, ap_uint<1> x4,
    ap_uint<1>& sum, ap_uint<1>& carry, ap_uint<1>& Cout) {
#pragma HLS inline
...
#pragma HLS inline
    ...
    // NAND present in approximate part (paper mentions NAND)
    sum = ~(p12 & p34);
    carry = p12;   // proxy; tweak if you want to match Fig. 6(a) exactly
    Cout = 0;     // C3 approximate part does not support Cout
}
...
#pragma HLS inline
    ...
    // higher-quality carry proxy (paper: carry improved vs C3)
    carry = (x1 & x2) | (x3 & x4);
    sum = p12 | p34;
    Cout = 0;
}
